                    ...
                    foreach (XmlNode node2 in nodes1)
                    {
                        ...
                        using (var db = new CreditinfoServiceEntities())
                        {
                            if (_ErrorText != "")
                            {
                                ...
                                customers.Add(new CreditInfoResult { Identifier = _Identifier, Result = _ErrorText, Date = DateTime.Now, IsML = 1, ErrorTypeId = ErrorStep });
        ...
        private static void UploadCreditCard()
        {
            try
            {
                using (var db = new CreditinfoServiceEntities())
                {
                    for (int i = 0; i < 20; i++)
                    {
                        var data = db.GetInfoForContracts_CreditCardML().ToList();
                        if (data.Count <= 0)
                            break;
                       // using (var client = new CreditinfoService.ServiceSoapClient())
                        {
                            ...
                            var template = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\ML\contract_CreditCard.xml");

                            var guid = Guid.NewGuid().ToString();

                            var fileName = @"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\ML\GeneritedContract_CreditCard\" + guid + ".xml";
                            float id = 1;
            ...
            */
            ...
            using (var db = new CreditinfoServiceEntities())
            {
                if (fin != 0)
                {
                    ...
                    Batch.Add(new CreditInfoBatchId { BatchId = fin, Date = DateTime.Now, IsML = 1 });
