    ...
    class Program
    {
        ...
        private static void Response(string ResponseID)
        {
            try
            {
                ...
                auth.Credentials = new System.Net.NetworkCredential("globusi.up", "X4Mg8T7q");
                    ...
                    foreach (XmlNode node2 in nodes1)
                    {
                        ...
                        using (var db = new CreditinfoServiceEntities())
                        {
                            if (_ErrorText != "")
                            {
                                ...
                                customers.Add(new CreditInfoResult { Identifier = _Identifier, Result = _ErrorText, Date = DateTime.Now, IsML = 0, ErrorTypeId = ErrorStep });
        ...
        private static void UploadCreditCard()
        {
            try
            {
                using (var db = new CreditinfoServiceEntities())
                {
                    for (int i = 0; i < 20; i++)
                    {
                        var data = db.GetInfoForContracts_CreditCard().ToList();
                        if (data.Count <= 0)
                            break;
                        // using (var client = new CreditinfoService.ServiceSoapClient())
                        {
                            var common = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\contract_outer.xml");
                            var template = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\contract_CreditCard.xml");

                            var guid = Guid.NewGuid().ToString();

                            var fileName = @"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\GeneritedContract_CreditCard\" + guid + ".xml";
                            float id = 1;
        ...
        private static void Upload()
        {
            try
            {
                using (var db = new CreditinfoServiceEntities())
                {
                    for (int i = 0; i < 20; i++)
                    {
                        var data = db.GetInfoForContracts_Intallment().ToList();
                        ...
                        {
                            var common = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\contract_outer.xml");
                            var template = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\contract.xml");
            ...
            try
            {
                using (var db = new CreditinfoServiceEntities())
                {
                    for (int i = 0; i < 20; i++)
                    {
                        ...
                        var data = db.GetInfoForContracts_NoNinstalment().ToList();
                        if (data.Count() <= 0)
                            break;
                        // using (var client = new CreditinfoService.ServiceSoapClient())
                        {
                            var common = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\contract_outer.xml");
                            var template = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\contract_Noninstalment.xml");

                            var guid = Guid.NewGuid().ToString();

                            var fileName = @"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\Generated_NonInstalment\" + guid + ".xml";
                            float id = 1;
                            foreach (var item in data)
                            {
        ...
        private static void UploadPersonalData()
        {
            try
            {
                using (var db = new CreditinfoServiceEntities())
                {
                    var data = db.GetPersonaData_intividual().ToList();
                    // using (var client = new CreditinfoService.ServiceSoapClient())
                    {
                        var common = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\contract_outer.xml");
                        var template = System.IO.File.ReadAllText(@"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\Individual.xml");

                        var guid = Guid.NewGuid().ToString();

                        var fileName = @"C:\Users\L.Shanava\Desktop\PAB\CreditInfo\Shablons\Globus\Generated_PersonalData\" + guid + ".xml";
                        float id = 1;
                        foreach (var item in data)
                        {
            ...
            */
            ...
            using (var db = new CreditinfoServiceEntities())
            {
                if (fin != 0)
                {
                    ...
                    Batch.Add(new CreditInfoBatchId { BatchId = fin, Date = DateTime.Now, IsML = 0 });
